...
- **目标与完成口径先和用户敲定。** 动笔前必须和用户确认这个 task 要达成什么、怎样算完成（验收口径），由用户拍板；你的活是把口径细化成可判定的 AC，不要自己假设验收标准。
- **默认精简。** 默认只填精简骨架（状态 / 目标 / 完成口径 / 交付物 / Non-goals / 复现入口 / 完成证据 / 开放问题）；重型代码 / 实验 / 数据任务才展开数据契约、Verifier 设计、反例测试、覆盖矩阵、Checkpoint 等。
...
**职责**：可勾选、可客观判断的完成条件，每条用 AC-ID 编号并绑 verifier / 反例 / 证据。**口径由用户定义**——把用户拍板的「怎样算完成」细化成 AC，别自己假设。轻量任务可把验证方式（grep / 人工审查）直接并进每条 AC，不必单列 Verifier 设计节。
